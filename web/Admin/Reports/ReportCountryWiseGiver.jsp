...
.CSSTableGenerator tr:first-child td:last-child{
    ...
    <body>
    <center><h2>Country Wise Giver</h2><br><br>
        <form>
    ...
    <div class="CSSTableGenerator">
        <table>
            ...
            <%
                ...
                ResultSet rs = dm.getData("SELECT `project_giver`.`Giver_id`, `project_giver`.`Giver_name`, `country`.`Country_name`, `project_giver`.`Giver_email`, `project_giver`.`Giver_password`, `project_giver`.`Giver_profile_pic`, `project_giver`.`Giver_status`, `project_giver`.`Giver_gender`, `project_giver`.`Giver_description` FROM `country` INNER JOIN `project_giver` ON (`country`.`Country_id` = `project_giver`.`Country_id`) where `project_giver`.`Country_id`=" + request.getParameter("txttechid")  + ";");
                while (rs.next()) {
                    out.println("<tr>");
